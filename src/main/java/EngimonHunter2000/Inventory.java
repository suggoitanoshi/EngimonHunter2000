...
class Inventory<T>{
  ...
  public void removeItem(T item) throws InventoryException{
    if(this.container.size() == 0) throw new InventoryException(1);
    if(this.container.contains(item)){
  ...
  public int getItemCount(){ return Inventory.itemCount; }
  
  public boolean isFull(){ return Inventory.itemCount == Inventory.max; }

  public static void resetItems(){ Inventory.itemCount = 0; }
}