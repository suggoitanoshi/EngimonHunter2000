...
TEST(Dex, SkillGetter) {
    ...
    EXPECT_EQ(dSkill.getElements()[0], ELECTRIC);
}
...
TEST(Dex, EngimonGetter) {
    ...
    EXPECT_EQ(dEngi.getElements()[0], ELECTRIC);
...
TEST(Dex, AddNonExistantSkill) {
    ...
    d.addSkill(skillName,
               Skill(skillName, 100, 1, vector<Elements>{FIRE, GROUND}));
    ...
    EXPECT_EQ(dSkill.getElements()[0], FIRE);
    EXPECT_EQ(dSkill.getElements()[1], GROUND);
}
...
TEST(Dex, AddExistingSkill) {
    ...
    EXPECT_THROW(d.addSkill(skillName, Skill(skillName, 100, 1,
                                             vector<Elements>{FIRE, GROUND})),
                 DexException);
}
...
TEST(Dex, AddNonExistantEngimon) {
    ...
    d.addEngi(engiName, EngimonSpecies(engiName, engiSlogan,
                                       vector<Elements>{FIRE, WATER}, Skill()));

    ASSERT_NO_THROW(d.getEngi(engiName));
    EngimonSpecies dEngi = d.getEngi(engiName);
    EXPECT_EQ(dEngi.getSpecies(), engiName);
    ASSERT_EQ(dEngi.getElements().size(), 2);
    EXPECT_EQ(dEngi.getElements()[0], FIRE);
    EXPECT_EQ(dEngi.getElements()[1], WATER);
...
TEST(Dex, AddExistingEngimon) {
    ...
    EXPECT_THROW(d.addEngi(engiName,
                           EngimonSpecies(engiName, "",
                                          vector<Elements>{ELECTRIC}, Skill())),
                 DexException);
}
